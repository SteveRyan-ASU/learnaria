;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype.setValue = function(n) {
		...
		if (n >= this.options.max) {
			...
			this.notification.text('Boom! Loading complete');
		} else {
			val = n;
		}
		
		this.element
			// .data({ 'value': parseInt(val) }) <- inaccessible
			// The following attribute is accessible.
			.attr({
				'aria-valuenow': val
			});
      ;
	...
	// allow the current value to be retrieved with keyboard controls. Triggers ARIA notify function for screen readers.
